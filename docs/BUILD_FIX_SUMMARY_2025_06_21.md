...
- **Issue**: Blog post images were not displaying on the homepage blog section
- **Root Cause**: GraphQL query in `src/pages/index.js` was missing the `featuredImage` field for WordPress posts
- **Solution**: Added `featuredImage { node { sourceUrl, altText } }` to the `allWpPost` query
- **Impact**: Blog posts now properly display featured images when WordPress is connected

### Content Character Limits

- **Issue**: Long blog titles and excerpts could break the homepage layout design
- **Solution**: Added intelligent text truncation functions in `src/components/blog-feature.js`:
  - `truncateTitle()`: Limits titles to 50 characters with word boundary preservation
  - `truncateHtmlText()`: Limits excerpt text to 120 characters while preserving HTML structure
- **Features**:
  - Smart word boundary cutting (no mid-word breaks)
  - HTML tag removal for accurate character counting
  - Ellipsis (...) added to truncated content
- **Impact**: Consistent layout regardless of content length, improved design stability
